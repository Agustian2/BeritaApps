import 'package:flutter/material.dart';

const kPrimaryColor = Color.fromRGBO(47, 137, 252, 1.000);
const kSecondaryColor = Color(0xFF59706F);
const kDarkGreyColor = Color(0xFFA8A8A8);
const kWhiteColor = Color.fromRGBO(245, 245, 245, 1.000);
const kZambeziColor = Color(0xFF5B5B5B);
const kBlackColor = Color.fromRGBO(64, 81, 78, 1.000);
const kTextFieldColor = Color(0xFF979797);

const lightBgColor = Color.fromRGBO(245, 245, 245, 1.000);
const lightFontColor = Color.fromRGBO(64, 81, 78, 1.000);
const lightDivColor = Color(0xffF5F6FA);
const lightPrimaryColor = Color.fromRGBO(47, 137, 252, 1.000);
const lightLableColor = Color(0xff8E8E8E);

const kDefaultPadding = EdgeInsets.symmetric(horizontal: 15);

TextStyle titleText =
    TextStyle(color: kPrimaryColor, fontSize: 32, fontWeight: FontWeight.w700);
TextStyle subTitle = TextStyle(
    color: kSecondaryColor, fontSize: 10, fontWeight: FontWeight.w500);
TextStyle textButton = TextStyle(
  color: kPrimaryColor,
  fontSize: 18,
  fontWeight: FontWeight.w700,
);
